...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  // Compute the sample weights 计算每个粒子的采样权重
  for (j = 0; j < set->sample_count; j++)
  {
    sample = set->samples + j;
    pose = sample->pose;
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      // This model ignores max range readings 将最大距离读数舍去
      if(obs_range >= data->range_max)
        continue;
      ...
      // Compute the endpoint of the beam 计算激光束的终点位姿
      hit.v[0] = pose.v[0] + obs_range * cos(pose.v[2] + obs_bearing);
      hit.v[1] = pose.v[1] + obs_range * sin(pose.v[2] + obs_bearing);

      // Convert to map grid coords. 将该点映射到map删格中
      int mi, mj;
      ...
      pz += self->z_hit * exp(-(z * z) / z_hit_denom); //以0为中心，self->sigma_hit 为标准方差的高斯分布距离概率
    ...
    sample->weight *= p; //每一个粒子的权重
    total_weight += sample->weight; //总的权重
  }

  return(total_weight); //观测模型返回权重
}
