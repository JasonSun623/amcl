...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  pf_vector_t old_pose = pf_vector_sub(ndata->pose, ndata->delta); //上一时刻的位姿
  ...
  case ODOM_MODEL_DIFF: //双轮差速运动模型
  {
    ...
    for (int i = 0; i < set->sample_count; i++) //对每一个粒子进行下一时刻位姿预测
    {
      pf_sample_t* sample = set->samples + i;

      // Sample pose differences 运动由第一次旋转，平移和第二次旋转近似
      ...
      // Apply sampled update to particle pose 由于每个粒子的噪声随机，因此位姿会在上一时刻位姿附近变动，从而预测出下一时刻的位姿
