...
class AmclNode
{
  public:
    ...
    void pf_init(pf_t* pf_, pf_vector_t pf_init_pose_mean, pf_matrix_t pf_init_pose_cov);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // Where was the robot when this scan was taken? 获得此时机器人base相对于odom的位姿
  pf_vector_t pose;
  ...
  if(pf_init_)//如果初始化完成，则通过判断相对位移和转角判断是否需要更新,update和 laser_update分别表示预测模型和观测模型是否需要更新
  {
  ...
  if(!pf_init_)  //通过初始化判断是否是第一次更新，如果第一次则将pf_init 和laser_update置为true
  {
  ...
  else if(pf_init_ && lasers_update_[laser_index]) //开始更新...
  {
    //printf("pose\n");
    //pf_vector_fprintf(pose, stdout, "%.3f");

    AMCLOdomData odata;//保存里程计数据，基于odo坐标系下的
    odata.pose = pose;
    // HACK
    ...
    // Use the action data to update the filter 更新运动学模型 概率机器人101页，里程计运动模型 
  ...
  // If the robot has moved, update the filter 更新观测模型，
  if(lasers_update_[laser_index])
  {
    ...
    //
    // Construct min and max angles of laser, in the base_link frame. 将laser_link下的激光数据转换到base_link坐标系下
    tf::Quaternion q;
    ...
    //更新观测模型。默认通过似然域计算后验概率，主要思想是将传感器扫描的终点z映射到地图的全局坐标系空间，获取到后验概率
    ...
    // Publish the resulting cloud 发布每个粒子的位姿
  ...
  if(resampled || force_publication)
  {
    ...
    // Read out the current hypotheses 找到权值最大的点并把该位姿发布出去
